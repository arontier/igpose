...
def identify_abag_with_hmm(abag_path, hmm_models_directory, tmp, pdb_id="foo", hmm_eval=float(1e-18), verbose=True, abseq_type_lookup=None):
    ...
    if abag_path.is_symlink():
        abag_path = abag_path.resolve()
    ...
    if heavy_chain or light_chain: return heavy_chain, light_chain, antigen_chains
    else: antigen_chains = [] 
...
def get_epitope_paratope_data(paired_residues, ag_chain, lc_or_hc, return_bio_pdb_aas = False):
    ...
    for pair in paired_residues:
        ...
        if not return_bio_pdb_aas:
            #get amino acid name'
            er = epi_res.get_resname()
            pr = para_res.get_resname()
            if not er in AA3to1_DICT or not pr in AA3to1_DICT:
                continue
            epi_res = AA3to1_DICT[er]
            para_res = AA3to1_DICT[pr]
