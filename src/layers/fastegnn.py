...
class FastEGNN(nn.Module):
    ...
    def __init__(self, node_feat_nf, node_feat_out_nf, edge_attr_nf, hidden_nf, virtual_channels,
        ...
        self.node_mlp = nn.Sequential(  # \phi_{h}
            nn.Linear(hidden_nf + hidden_nf + virtual_channels * hidden_nf, hidden_nf),
            act_fn,
    ...
    def edge_model(self, source, target, radial, edge_attr=None):
        tensors = [source, target, radial]
        if not edge_attr is None:
            tensors.append(edge_attr)
    ...
    def coord_model_real(self, coord, edge_index, coord_diff, edge_feat, virtual_edge_feat, virtual_coord_diff):
        row, _ = edge_index
        
        trans = coord_diff * self.coord_mlp_r(edge_feat)  # coord_mlp_r: [batch_edge, H] -> [batch_edge, 1]
        if self.coords_agg == 'sum':
            agg = unsorted_segment_sum(trans, row.to(torch.int64), num_segments=coord.size(0))
        elif self.coords_agg == 'mean':
            agg = unsorted_segment_mean(trans, row.to(torch.int64), num_segments=coord.size(0))
        else:
    ...
    def node_model(self, node_feat, edge_index, edge_feat, virtual_edge_feat):
        # node_feat: [batch_node, H], edge_feat: [batch_edge, H], virtual_edge_feat: [batch_node, H, C]
        row, col = edge_index
        agg = unsorted_segment_mean(edge_feat, row.to(torch.int64), num_segments=node_feat.size(0))  # [batch_node, H]
    ...
    def forward(self, node_feat, edge_index, coord, virtual_coord, virtual_node_feat, data_batch, edge_attr=None):
        '''
        ...
        # Coord Model
        coord = self.coord_model_real(coord, edge_index, coord_diff, edge_feat, virtual_edge_feat, virtual_coord_diff)
        virtual_coord = self.coord_model_virtual(virtual_coord, virtual_edge_feat, virtual_coord_diff, data_batch)
        # Node Model
        node_feat = self.node_model(node_feat, edge_index, edge_feat, virtual_edge_feat)
...
# class FastEGNN(nn.Module):
#     def __init__(self, node_feat_nf, edge_attr_nf, hidden_nf, virtual_channels, act_fn=nn.SiLU(), 
...
#                         phi_x(m_ij) which definitely improves in stabaility but it may decrease in accuracy.
#                         We didn't use it in our paper.
#         '''
...
#             self.add_module("gcl_%d" % i, E_GCL_vel(hidden_nf, hidden_nf, edge_attr_nf, hidden_nf, virtual_channels=virtual_channels,
#                                                     act_fn=act_fn, residual=residual, attention=attention, normalize=normalize, tanh=tanh))

#     def forward(self, node_feat, edge_index, edge_attr=None, pos=None, loc_mean=None, data_batch=None, batch_size=None):
...
#             node_feat, real_node_loc, virtual_node_feat, virtual_node_loc = \
#                   self._modules["gcl_%d" % i](node_feat, edge_index, real_node_loc, virtual_node_loc, virtual_node_feat, 
#                                                 data_batch, edge_attr=edge_attr)
#         return real_node_loc, virtual_node_loc